...
func (t *task) followUpMyAccount(redirect *string, headers http.Header) (Result, error) {
...
func ZalandoInit(t *utils.Task) {
	...
	res, err = z.retryLogic("FollowRedirect", func() (Result, error) {
		res, e := z.followUpMyAccount(&redirect, headersAuthorize)
		return res, e
	})
	if err != nil {
		utils.LogError(z.TaskNumber, "followUpMyAccount", fmt.Sprintf("final status=%d msg=%q, location=%s", res.Status, res.Msg, res.Location), err)
		return
	}

	redirect = res.Location
	res, err = z.retryLogic("followUpMyAccount", func() (Result, error) {
		res, e := z.followUpMyAccount(&redirect, headersAuthorize)
		return res, e
	})
	if err != nil {
	...
	res, err = z.retryLogic("followUpMyAccount", func() (Result, error) {
		res, e := z.followUpMyAccount(&redirect, headersAuthenticate)
		return res, e
	})
	if err != nil {
		utils.LogError(z.TaskNumber, "followUpMyAccount", fmt.Sprintf("final status=%d msg=%q, location=%s", res.Status, res.Msg, res.Location), err)
	...
	if err != nil {
		utils.LogError(z.TaskNumber, "followUpMyAccount", fmt.Sprintf("final status=%d msg=%q", res.Status, res.Msg), err)
		return
	}
